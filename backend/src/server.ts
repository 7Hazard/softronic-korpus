...
export async function start({
    ...
    app.post("/synonyms", async (req,res) => {
        ...
        } else res.status(400).json("No circular or transitive dependencies allowed");
    ...
    app.put("/synonyms", async(req,res)=>{
        ...
        if(await Words.get(IdToChange) == undefined || await Words.get(oldId2) == undefined ||await Words.get(newId2) == undefined){
        ...
        try {
            ...
            if(await Synonyms.isValidInput(IdToChange,newId2,oldId2)){
                await database.get().getRepository(Synonym)
